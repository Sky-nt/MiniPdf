...
MiniPdf 的输出与 LibreOffice 作为参考渲染器进行对比，涵盖 **180 个经典测试用例**（包含 30 个图片嵌入测试、30 个图表测试、30 个样式测试和 30 个多语言/表情符号测试）。
...
**整体平均分: 96.5%**（文本相似度 40% + 视觉相似度 40% + 页数 20%）

### 视觉对比

所有测试用例对比 MiniPdf 输出与 LibreOffice 参考。多页结果仅显示第 1 页。

<table>
<tr><th>MiniPdf</th><th>LibreOffice（参考）</th></tr>
<tr>
  <td><b>classic01</b></td>
...
MiniPdf DOCX 的输出与 LibreOffice 作为参考渲染器进行对比，涵盖 **180 个经典测试用例**。
...
所有 DOCX 测试用例对比 MiniPdf 输出与 LibreOffice 参考。多页结果仅显示第 1 页。
