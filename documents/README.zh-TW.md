...
- **嵌入圖片** — Excel 工作表中嵌入的 JPEG 和 PNG 圖片會渲染到 PDF 輸出中
...
MiniPdf 的輸出與 LibreOffice 作為參考渲染器進行對比，涵蓋 **180 個經典測試案例**（包含 30 個圖片嵌入測試、30 個圖表測試、30 個樣式測試和 30 個多語言/表情符號測試）。
...
**整體平均分: 96.5%**（文字相似度 40% + 視覺相似度 40% + 頁數 20%）

### 視覺對比

所有測試案例對比 MiniPdf 輸出與 LibreOffice 參考。多頁結果僅顯示第 1 頁。

<table>
<tr><th>MiniPdf</th><th>LibreOffice（參考）</th></tr>
<tr>
  <td><b>classic01</b></td>
...
MiniPdf DOCX 的輸出與 LibreOffice 作為參考渲染器進行對比，涵蓋 **180 個經典測試案例**。
...
所有 DOCX 測試案例對比 MiniPdf 輸出與 LibreOffice 參考。多頁結果僅顯示第 1 頁。
