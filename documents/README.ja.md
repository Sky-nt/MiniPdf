...
MiniPdf の出力は、LibreOffice を参照レンダラーとして **120 のクラシックテストケース**（画像埋め込み 30 件、グラフ 30 件を含む）と比較されています。
...
**総合平均スコア: 94.7%** （テキスト類似度 40% + 視覚的類似度 40% + ページ数 20%）

### 視覚比較

MiniPdf 出力と LibreOffice 参照を比較した全テストケース。複数ページの場合は 1 ページ目を表示。
